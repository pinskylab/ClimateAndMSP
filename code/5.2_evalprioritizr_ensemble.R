# Evaluate the prioritzr solutions against the ensemble of species projections
...
for (i in 1:length(rcps)){
    ...
    for(j in 1:length(oceans)){
        for(k in 1:length(periods)){
            ...
            # calculate ensemble mean for training or testing set of GCMs
            prestemptrain <- prestemp[model %in% c(1:16)[-gcminds], .(poccur = mean(poccur)), by = c('latgrid', 'longrid', 'year_range', 'rcp', 'spp')]
            biotemptrain <- biotemp[model %in% c(1:16)[-gcminds], .(biomass = mean(biomass)), by = c('latgrid', 'longrid', 'year_range', 'rcp', 'spp')]
            prestemptest <- prestemp[model %in% c(1:16)[gcminds], .(poccur = mean(poccur)), by = c('latgrid', 'longrid', 'year_range', 'rcp', 'spp')]
            biotemptest <- biotemp[model %in% c(1:16)[gcminds], .(biomass = mean(biomass)), by = c('latgrid', 'longrid', 'year_range', 'rcp', 'spp')]
                        
            prestemptrain[, model := 'train']
            biotemptrain[, model := 'train']
            prestemptest[, model := 'test']
            biotemptest[, model := 'test']
            
            if(i == 1 & j == 1 & k == 1){
                presmap <- rbind(prestemptrain, prestemptest)
                biomassmap <- rbind(biotemptrain, biotemptest)
            } else {
                presmap <- rbind(presmap, prestemptrain, prestemptest)
                biomassmap <- rbind(biomassmap, biotemptrain, biotemptest)
            }
        }
    }
}
rm(prestemp, biotemp, prestemptrain, biotemptrain, prestemptest, biotemptest)
    
    # optional plots to examine presmap
    # thisspp <- 'gadus morhua'
    # thisspp <- 'urophycis chuss'
    # ggplot(presmap[spp == thisspp & model == 'test' & rcp == 85], 
    #        aes(x = longrid, y = latgrid, color = poccur > 0.5)) +
    #     geom_point(size = 0.2) +
    #     facet_wrap(~ year_range, ncol = 2)
    # 
    # ggplot(biomassmap[spp == thisspp & model == 'test' & rcp == 85], 
...
######################################################################
# Analyze plans against each climate ensemble projection             #
# across each rcp                                                    #
######################################################################
...
# set up ensemble types for evaluation
modelsp <- sort(unique(presmap$model))
modelsb <- sort(unique(biomassmap$model))
if(any(modelsp != modelsb)) stop('biomass and presmap types do not match')

# set up time periods for evaluation
periods <- sort(unique(presmap$year_range))

# evaluate # targets met in each time period in each ensemble type
...
goalsmetbymod1out <- goalsbyzonebymod1out[!is.na(metgoal), .(mid = mean(as.numeric(unlist(strsplit(as.character(year_range), split='-')))), 
                                                             ngoals = .N, nmet = sum(metgoal), pmet = sum(metgoal)/.N), 
                                           by = c('year_range', 'model', 'rcp', 'region')]
goalsmetbymod2out <- goalsbyzonebymod2out[!is.na(metgoal), .(mid = mean(as.numeric(unlist(strsplit(as.character(year_range), split='-')))), 
                                                             ngoals = .N, nmet = sum(metgoal), pmet = sum(metgoal)/.N), 
                                           by = c('year_range', 'model', 'rcp', 'region')]
...
# write out
write.csv(goalsbyzonebymod1out, file = gzfile(paste0('output/goalsbyzonebyensemble_', runname1out, '.csv.gz')))

...
# Run twice: once using only historical distributions, once using historical + future distributions
# May need to set R_MAX_VSIZE=60000000 or larger in .Renviron to avoid hitting memory limits
...
## Load data
#####################
# define ocean so that we can read in the correct projection files
...
pus <- presmap[region == myreg, c('latgrid', 'longrid')]
...
# fishery features entered separately from conservation features, even if same species
	sppstokeep <- presmap[region == myreg & year_range == planningperiods[1] & rcp %in% rcps & model %in% c(1:11, 13:15, 17:18), .(poccur = mean(poccur)), by = c('latgrid', 'longrid', 'spp')] # average across models
		dim(sppstokeep)
	...
	sppstokeep <- merge(sppstokeep, pus[, .(latgrid, longrid, id)], by = c('latgrid', 'longrid')) # add pu id (and trim to focal pus)
		...
		sppstokeep[ , length(unique(spp))] # 179 (ebs), 161 (goa), (bc),  (wc), 195 (gmex), (seus), (neus), (maritime), (newf)
	...
	spps <- rbind(spps, data.table(id = max(spps$id) + 1:fisheryspps[region == myreg, length(projname)], 
	                               name = paste0(gsub(' |_', '', fisheryspps[region == myreg, projname]), '_fishery'),
	                               spp = fisheryspps[region == myreg, projname]))
...
# which features are in each planning unit
    ...
    # checks
	length(unique(puvsp$pu)) # planning units for species + NatCap: 2195 (ebs), 661 (goa), 549 (neus), 1342 (newf)
	length(unique(puvsp$species)) # features that are species + fishery + NatCap: 190 (ebs), 172 (goa), 68 (neus), 71 (newf)
	puvsp[, summary(amount)] # range of amounts
	sort(unique(table(puvsp$species))) # make sure all species show up in some planning units (shouldn't see any 0s)
	sort(unique(table(puvsp$pu))) # make sure all planning units have some species (shouldn't see any 0s)
	sort(unique(table(puvsp$pu, puvsp$species))) # should be all 0s and 1s
...
nrow(spps) # the # of features
all(rownames(zonetarget) == spps$name) # target order matches species?
sum(!(puvsp$pu %in% pus$id)) # any planning units not in pus?
sum(!(puvsp$species %in% spps$id)) # any species not in spps?
sum(!(pus$id %in% puvsp$pu)) # any pus not in puvsp?
sum(!(spps$id %in% puvsp$species)) # any species not in puvsp?
	

# Define the problem in prioritzr format
# rij may need a zone column
p1 <- problem(pus, spps, cost_column = c('dummycost', 'dummycost', 'dummycost'), 
              rij = puvsp, zones = zones) %>%
...
# use only first 8 models, leaving later 8 for testing
	...
	puvsppa2 <- presmap[region == myreg & year_range == planningperiods[2] & rcp %in% rcps & model %in% c(1:11, 13:15, 17:18)[gcminds], .(poccur = mean(poccur)), by = c('latgrid', 'longrid', 'spp')] # pres/abs data. climate models are bcc-csm1-1-m, bcc-csm1-1, CanESM2, CCSM4, CESM1-CAM5, CNRM-CM5, GFDL-CM3, GFDL-ESM2M, GFDL-ESM2G, GISS-E2-R, GISS-E2-H, HadGEM2-ES, IPSL-CM5A-LR, IPSL-CM5A-MR, MIROC-ESM, MIROC5, MPI-ESM-LR, NorESM1-ME. only use those that match biomass projections
		dim(puvsppa2)
    ...
    # Format fishery data
    puvspbio2 <- biomassmap[region == myreg & year_range == planningperiods[2] & rcp %in% rcps & model %in% (1:16)[gcminds] & spp %in% fisheryspps[region == myreg, projname], .(biomass = mean(biomass)), by = c('latgrid', 'longrid', 'spp')] # biomass data. models are bcc-csm1-1-m, bcc-csm1-1, CanESM2, CCSM4, CESM1-CAM5, CNRM-CM5, GFDL-CM3, GFDL-ESM2M, GFDL-ESM2G, GISS-E2-R, GISS-E2-H, IPSL-CM5A-LR, IPSL-CM5A-MR, MIROC-ESM, MPI-ESM-LR, NorESM1-ME
        dim(puvspbio2)
	...
	temp1[!grepl('energy', name), name := paste0(name, gsub('-', '', planningperiods[1]))] # append time period
	puvsp2 <- rbind(temp1, puvsp2)

    # checks
	length(unique(puvsp2$pu)) # planning units for species + NatCap: 2195 (ebs), 549 (neus), 1342 (newf)
	length(unique(puvsppa2$name)) # cons features that are species: 303 (ebs), 68 (neus), 71 (newf)
	length(unique(puvsp2$species)) # features that are species + fishery + NatCap: 340 (ebs), 68 (neus), 71 (newf)
	setdiff(spps2$name, puvsp2$name) # species not present as features
	puvsp2[, summary(amount)] # range of amounts
	sort(unique(table(puvsp2$species))) # make sure all species show up in some planning units (shouldn't see any 0s)
	sort(unique(table(puvsp2$pu))) # make sure all planning units have some species (shouldn't see any 0s)
	sort(unique(table(puvsp2$pu, puvsp2$species))) # should be all 0s and 1s
...
# set zone-specific targets: rows are features, columns are zones
	...
	# set conservation zone target
	zonetarget2[!grepl('energy|fishery', rownames(zonetarget)), 'conservation'] <- consgoal # XX proportion

	# set fishing zone target
	zonetarget2[grepl('fishery', rownames(zonetarget)), 'fishery'] <- fishgoal # XX proportion
	
	# set energy goal target
	zonetarget2[grepl('energy', rownames(zonetarget)), 'energy'] <- energygoal # XX proportion

# trim out species that aren't present
	nrow(spps2)
	spps2 <- spps2[spps2$name %in% puvsp2$name,]
	nrow(spps2) # 340 (ebs), 314 (goa)
...
# basic checks
colSums(zonetarget2) # reasonable targets?
dim(zonetarget2) # should match # features
nrow(spps2) # the # of features
all(rownames(zonetarget2) == spps2$name) # target order matches species?
sum(!(puvsp2$pu %in% pus$id)) # any planning units not in pus?
sum(!(puvsp2$species %in% spps2$id)) # any species not in spps?
sum(!(pus$id %in% puvsp2$pu)) # any pus not in puvsp?
sum(!(spps2$id %in% puvsp2$species)) # any species not in puvsp?
...
# write out
write.csv(s2, file = paste0(marxfolder, 'solution_', runname2, '.csv'))
